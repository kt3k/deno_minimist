...
describe("Positional Arguments", () => {
  ...
  for (let o of options) {
    ...
    it(`post -- arguments match expected order (${o.description})`, () => {
      ...
      if (!o.options["--"]) {
        expect(args._).contains.ordered.members([
          "abc",
          "efg",
          "hij",
          "klm",
    ...
    it(`arguments after flags match expected order (${o.description})`, () => {
      const args = index(
        ["abc", "-d", "-e", "true", "efg", "hij", "--", "klm", "nop", "qrs"],
        o.options
      );
      if (!o.options["--"]) {
        expect(args._).contains.ordered.members([
...
describe("Flags", () => {
  ...
  it(`many flags with no value parses as true`, () => {
    ...
    expect(args)
      .has.property("one")
      .which.is.true.and.is.a("boolean");
    expect(args)
      .has.property("two")
      .which.is.true.and.is.a("boolean");
    expect(args)
  ...
  it(`default option with no argument present as expected`, () => {
    ...
    expect(args)
      .has.property("false")
      .which.is.false.and.is.a("boolean");
  });
...
describe("ParseOption.boolean", () => {
  it(`true causes double-hyphen with = arguments to match as boolean`, () => {
    ...
    const args = index(["--one=true", "--two=false", "--three=asdf"], options);
...
describe("ParseOption.alias", () => {
  ...
  it(`alias with string array sets values correctly with single-hyphen flags`, () => {
    ...
    expect(args)
      .has.property("what-does-the-fox-say")
      .and.equals("ninini")
      .and.is.a("string");
    expect(args)
      .has.property("w")
...
describe("ParseOption.unknown", () => {
  it(`return false unknown hides unknown values`, () => {
    ...
    const args = index(
      ["--one", "one", "--two=two", "--three", "--four=false"],
      options
    );
    expect(args)
      .has.property("one")
