    ...
    if (parsing && arg.length > 1) {
      if (arg.startsWith("--")) {
        ...
        if (arg.indexOf("=") > -1) {
          ...
          i + 1 < actual_args.length &&
          (actual_args[i + 1] == "-" || !actual_args[i + 1].startsWith("-")) &&
          options.matches_type(arg.substr(2), actual_args[i + 1], true)
        ) {
        ...
        if (
          ...
          i + 1 < actual_args.length &&
          (actual_args[i + 1] == "-" || !actual_args[i + 1].startsWith("-")) &&
          options.matches_type(arg.substr(1), actual_args[i + 1])
        ) {
